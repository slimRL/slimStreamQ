...
def sparse_cnn_uniform(key, shape, dtype):
    ...
    kernel = nn.initializers.variance_scaling(1 / 3, "fan_in", "uniform", in_axis=(0, 1, 2), out_axis=(0, 1, 3))(
        uniform_key, shape, dtype
    )
    return kernel * jax.random.bernoulli(mask_key, 1 - 0.9, shape)


def sparse_mlp_uniform(key, shape, dtype):
    uniform_key, mask_key = jax.random.split(key)
    kernel = nn.initializers.variance_scaling(1 / 3, "fan_in", "uniform")(uniform_key, shape, dtype)
    return kernel * jax.random.bernoulli(mask_key, 1 - 0.9, shape)
